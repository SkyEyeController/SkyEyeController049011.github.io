...
{% asset_img 1-1-1.png 终端Powershell %}
...
{% asset_img 1-1-2.png 安装 %}

安装完成后，系统会自己启动Ubuntu18.04.

{% asset_img 1-1-3.png Ubuntu正确启动 %}

这个界面是正常的，但如果你出现了以下界面：

{% asset_img 1-1-4.png 错误0x800701bc %}
...
{% asset_img 1-1-5.png win10及以上启动控制面板 %}
...
```
会进入以下界面：
{% asset_img 1-1-6.png sources.list %}
...
{% asset_img 1-1-7.png 非insert模式 %}

按Shift+：进入命令模式，输入wq。
{% asset_img 1-1-8.png 冒号与wq指令 %}
...
{% asset_img 1-2-1.png WSL扩展下载 %}

安装完之后如图1-2-1，左下角会出现一个绿色的方框，里面有两个箭头，表示远程连接。
{% asset_img 1-2-2.png 绿色框 %}

点击绿色方框，选择“NEW WSL Window”，这时会弹出来一个新的VScode窗口，等其初始化完毕即可完成。

{% asset_img 1-2-3.png NEW WSL Window（或connect to WSL） %}

{% asset_img 1-2-4.png 加载中 %}

{% asset_img 1-2-5.png 成功 %}
...
{% asset_img 3-1-1.png Git Bash Here %}

{% asset_img 3-1-2.png Git Bash 窗口 %}
...
## 4.1 GitHub账号注册与仓库建设
  ...
  {% asset_img 4-1-1.png GitHub注册界面1 %}

  {% asset_img 4-1-2.png GitHub注册界面2 %}

  期间会有输入邮箱账号、设置密码、验证码、个性化（个性化看个人设置即可）

  注册完成并登录后，点击左侧的NEW，新建仓库。

  {% asset_img 4-1-3.png GitHub界面 %}

  {% asset_img 4-1-4.png GitHub仓库建设 %}
  ...
  {% asset_img 4-3-2.png SSH-Key界面 %}

  点击 Add Deploy Key 准备添加。

  {% asset_img 4-3-3.png SSH Key添加 %}
...
[虚仿平台使用手册](http://rqzta2283.sabkt.gdipper.com/%E9%99%84%E4%BB%B61-%E8%99%9A%E6%8B%9F%E4%BB%BF%E7%9C%9F%E5%AE%9E%E9%AA%8C%E5%B9%B3%E5%8F%B0%E4%BD%BF%E7%94%A8%E6%89%8B%E5%86%8C.pdf)
 ...
 {% asset_img 5-2-1.png 远程连接 %}

 在Vscode里安装插件Remote-SSH：

 {% asset_img 5-2-2.png Remote-SSH %}

 这时左侧有远程资源管理器Logo，点击进入，选择远程-->SSH设置小齿轮

  {% asset_img 5-2-4.png 远程 %}
  {% asset_img 5-2-3.png 小齿轮 %}

  在弹出框选择一个路径配置信息（我就选的之前的.ssh密钥钥文件夹）

  {% asset_img 5-2-5.png 路径 %}
...
{% asset_img 5-2-6.png 链接 %}

点击connect（——>）按钮，会弹出一个新的Vscode窗口，期间会有对操作系统的询问以及对密码Simplexue123的需求。

{% asset_img 5-2-7.png 询问 %}

{% asset_img 5-2-8.png 确认密码 %}
